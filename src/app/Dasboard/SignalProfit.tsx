...
const SignalProfit = () : JSX.Element =>{
    ...
    React.useEffect(() =>{
        setTimeout(async () =>{
            try{
                const fetchSignal = await fetch(`${dev_api.API_URL}api/signal/getprofit`, {
                    method: 'GET',
                    headers: new Headers({
    ...
    return(
        <>
            <Row>
                {!signal.loading && <CardSkeleton card={4} size={4}/>}
                {
